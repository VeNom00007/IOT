...
main (int argc, char *argv[])
{
  ...
  NetDeviceContainer devContainer = lrWpanHelper.Install(n);
  ...
  lrWpanHelper.EnableAsciiAll (ascii.CreateFileStream ("scratch/udp-client-server.tr"));
  lrWpanHelper.EnablePcapAll (std::string ("scratch/udp-client-server"), true);
  
  // animation using NetAnim
  AnimationInterface anim("scratch/q1.xml");
  int j = 0;
  for(float i=10.0;i<100;i+=20.0){
    anim.SetConstantPosition(n.Get(j),i,20.0);
    anim.SetConstantPosition(n.Get(9-j),i,60.0);
    j++;
  }
