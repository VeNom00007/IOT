...
#define UDP_PORT_CENTRAL 7279
#define UDP_PORT_OUT 5555
#define CLOCK_REPORT CLOCK_SECOND*2
static struct simple_udp_connection broadcast_connection;
static uip_ipaddr_t server_addr;
static uint16_t central_addr[] = {0xaaaa, 0, 0, 0, 0, 0, 0, 1};
...
PROCESS_THREAD(init_system_proc, ev, data){
		...
		int temp = (rand()+30)%45;
                sprintf((char *)buff_udp, "temp=%d,addr:%02X%02X|%ddBm|%s",temp,linkaddr_node_addr.u8[6],linkaddr_node_addr.u8[7],def_rt_rssi,def_rt_str);
